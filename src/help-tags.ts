export const about = [
    {
        ...
        sections: {
            ntcStructuralAlphabet: {
                headline: 'The NtC structural alphabet',
                paragraph1: 'The DNATCO server relies on the NtC structural alphabet, comprising 96 symbols that represent unique dinucleotide conformers. These conformers are determined by assessing 12 specific backbone torsion parameters (as depicted in Figure 1), which characterise the conformation of the DNA/RNA backbone.',
                paragraph2: 'Figure 1: The 12 parameters defining the NtC conformational class are illustrated here, with backbone torsions in grey and distances in blue, in the context of a dinucleotide step.',
                paragraph3: 'The NtC structural alphabet, developed within our institute, serves as a valuable tool for the analysis of nucleic acid structures, encompassing both DNA and RNA. This structural alphabet employs a set of 96 symbols, each comprising four characters (e.g., BB00 or ZZ1S), and these symbols represent distinct dinucleotide conformers. Detailed explanation for the nomenclature of NtC conformers is provided below. A comprehensive table listing all NtC conformers is accessible for download via the "ABOUT > Downloads" tab. Additionally, users have the option to download the definitions of NtC conformers, including torsion averages and esd values, along with representative structures of these conformers in cartesian coordinates.',
            },
            ntcNamingRules: {
                headline: 'NtC naming rules',
                paragraph1: 'The NtC conformers are denoted by four-character symbols. Conformers whose symbols commence with "A," "B," or "Z" in the first and/or second position correspond to dinucleotides featuring stacked bases, with the initial or second nucleotide adopting an A-, B-, or Z-like conformation, respectively (e.g., AA00, BB00 exemplifying the canonical A-form and B-form structures). Conformers with names beginning with "IC" represent steps with bases that are parallel but distanced and can be intercalated, while those starting with "OP" pertain to steps with unstacked "open" bases. Symbols featuring "S" in the 3rd or 4th position indicate that the first or second base, respectively, is in the syn orientation. Conformers that deviate significantly from the categories above are designated as the 97th conformer and are formally referred to as "NANT."'
            },
            glossaryOfAcronyms: {
                headline: 'Glossary of acronyms',
                paragraph1: 'NtC, short for the diNucleotide Conformers, is a structural alphabet that characterises dinucleotide conformers based on 12 torsion and distance parameters. It categorises these conformers into 96+1 classes, with the final class reserved for those that have not been specifically assigned.',
                paragraph2: 'CANA, the Conformational Alphabet of Nucleic Acids, clusters similar NtCs together and consists of 14+1 symbols. For more details on NtCs and CANA letters, please refer to the "Browse > Table of Conformers" section.',
                paragraph3: `CS, which stands for the "Confal Score," is a quality metric. A score of 100 indicates a perfect alignment with a reference, while a score of 0 signifies a significant departure from the expected conformation. The confal function follows a Gaussian distribution, peaking at 100 at the parameter's average value and tapering off to 1 as it approaches the boundary closer to the average. Values further from the average are assigned a confal score of 0. To determine the confal score for a given step, it's calculated as the harmonic mean of its 12 confal values. For an entire structure, the confal score is derived as the average of the individual step values.`,
                paragraph4: 'RMSD, or the Root Mean Square Deviation, quantifies the average spatial difference in cartesian coordinates between the analysed step and a reference.',
                paragraph5: 'RSCC, or the real-space correlation coefficient, serves as a metric for quantifying the likeness between an electron-density map derived directly from a structural model and one computed from experimental data.',
            }
        }
    }
]
...
export const  refinement = [
    {
        ...
        sections: {
            connectivityPlot: {
                ...
                paragraph1: 'The "Connectivity Plot" tab features three distinct plots, including the similarity plot, which is also accessible through the "validation" page, and two connectivity plots. The connectivity plots are designed to illustrate how well-connected the structure would be if the components of the structure making up the previous, currently selected, and next steps were all replaced by reference NtCs.',
                paragraph2: `To calculate connectivity with the previous step, DNATCO assesses the positions of the C5’ and O3’ atoms of the first residue in the currently selected step and compares them to the positions of the C5’ and O3’ atoms of the second residue in the previous step. The distances between these C5’ and O3’ atoms are represented in the connectivity plot. Similarly, connectivity to the next step is calculated, but it involves the distances between the C5’/O3’ atoms of the second residue in the current step and the C5’/O3’ atoms of the first residue in the next step. DNATCO computes connectivities between the currently selected NtC of the current step and all reference NtCs applied to the previous and next steps.`,
            },
            restraints: {
                headline: 'Restraints',
                paragraph1: 'The "REFMAC Restraints" tab presents a list of restraints that can be utilised with the REFMAC software. These restraints are generated from the currently selected set of NtCs.',
                paragraph2: `Similarly, the "Phenix Restraints" tab offers a set of restraints suitable for use with the Phenix software. It's important to note that the restraints file generated by DNATCO is compatible with a modified version of Phenix.`,
                paragraph3: `Additionally, the "MMB Commands File" tab provides a snippet of a commands file for the MacroMolecule Builder (MMB) software. Integrating this snippet into an MMB commands file instructs MMB to attempt to align the structure's geometry with the specified NtCs.`
            },
            changeNtCs: {
                headline: 'Change NtCs',
                paragraph1: 'The "Change NtCs" tab serves as a utility for users to directly adjust the assigned NtCs. While the default set of NtC conformers assigned by DNATCO cannot be altered, users have the flexibility to create multiple custom NtC sets, which can be modified and reviewed using the plots in the other tabs. Additionally, restraints for these custom NtC sets can be downloaded for further usage.'
            }
        }
    }
]
