...
const Tabs = [
  [
    "browse-conformers",
    {
      caption: "Conformers",
      title:
        "Search PDB database for dinucleotide steps of given conformation (NtC)",
    },
  ],
  ["base-pairs", { caption: "Base pairs", title: "Base pairs" }],
  [
    "table-of-conformers",
    { caption: "Table of conformers", title: "Table of conformers" },
  ],
  [
    "contour-plots",
    ...
    <Help.Container>
      {displayBrowse.map((page: any, index: any) => (
        <>
          <div
            key={index}
            className="flex border-t-secondary-second border-t pt-3 mb-8"
          >
            <div className="w-[25%]">
...
class BrowseConformers extends React.Component {
  ...
  private renderStepsTable() {
    const names: DynamicTable.Column<string> = {
      name: "Name",
      cells: new Array<DynamicTable.Cell<string>>(),
    };
    const CANAs: DynamicTable.Column<string> = {
      name: "CANA",
      cells: new Array<DynamicTable.Cell<string>>(),
    };
    const NtCs: DynamicTable.Column<string> = {
      name: "NtC",
      cells: new Array<DynamicTable.Cell<string>>(),
    };
    const confals: DynamicTable.Column<number> = {
      name: "Confal",
      cells: new Array<DynamicTable.Cell<number>>(),
    };
    const rmsds: DynamicTable.Column<number> = {
      name: "RMSD",
      cells: new Array<DynamicTable.Cell<number>>(),
    };
    const resolutions: DynamicTable.Column<number> = {
      name: "Resolution [\u00C5]",
      cells: new Array<DynamicTable.Cell<number>>(),
    };
    const haveMaps: DynamicTable.Column<string> = {
      name: "Map",
      cells: new Array<DynamicTable.Cell<string>>(),
    };
    ...
    for (const step of this.search.results) {
      names.cells.push({
        data: step.name,
        elem: (
          <div className="rdo-found-conformers-stepname-cell">{step.name}</div>
        ),
      });
    ...
    const model = new DynamicTable.Model([
      names,
      CANAs,
      NtCs,
      confals,
    ...
    const download =
      this.search.results.length > 0
        ? {
            ...
            fileName: `search_${this.search.criteria.NtC}_count_${
              this.search.criteria.maxCount
            }_${
              this.search.criteria.largeStructures ? "with" : "without"
            }_large_${
              this.search.criteria.redundant ? "with" : "without"
            }_redundant`,
          }
        : undefined;
  ...
  searchConformers = async (criteria: Search.Criteria) => {
    ...
    const p = Search.requestSearch(
      criteria.NtC,
      criteria.maxCount,
      criteria.redundant,
      criteria.largeStructures
    );
  ...
  private renderList() {
    if (!ListOfConformers.has()) {
      if (ListOfConformers.failed())
        return (
          <div className="text-secondary-third">{`List of conformers failed to load: ${ListOfConformers.fail}`}</div>
        );
    ...
    return (
      <table className="rdo-list-of-conformers rdo-data-table-wide w-full">
        <thead>
          <tr>
            <th
              className="border-b border-solid border-primary-first"
              rowSpan={2}
            >
              NtC
            </th>
            <th
              className="border-b border-solid border-primary-first border-r"
              rowSpan={2}
            >
              CANA
            </th>
            ...
            <th
              className="border-r border-solid border-primary-first"
              colSpan={5}
            >
              Number of steps in
            </th>
            <th
              className="border-b border-solid border-primary-first bg-[#FBEDDA]"
              rowSpan={2}
            >
              δ1
            </th>
          ...
          <tr>
            <th className="border-b border-solid border-primary-first">
              {"N\u00A0DNA"}
            </th>
            <th className="border-b border-solid border-primary-first">
              {"%\u00A0DNA"}
            ...
            <th className="border-r border-b border-solid border-primary-first">
              {"%\u00A0RNA"}
            </th>
            <th className="border-r border-b border-solid border-primary-first">
              {"N\u00A0GS"}
        ...
        <tbody>
          {ListOfConformers.list.map((info, idx) => {
            ...
            return (
              ...
              >
                ...
                <td className="text-center border-r border-solid border-primary-first">
                  {info.CANA}
                </td>
                <td className="border-r border-solid border-primary-first">
                  {info.description}
                </td>
                ...
                <td className="border-r border-solid border-primary-first text-right">
                  {fmtFlt(info.percentInRNA)}
                </td>
                <td className="border-r border-solid border-primary-first text-right">
                  {fmtInt(info.countGS)}
                </td>
                <td
                  className={`text-right rdo-list-of-conformers-highlighted-${
                    info.highlight ? "row" : "col"
                  }`}
                >
                  {fmtInt(info.delta1)}
                </td>
          ...
          <tr>
            ...
            <td className="border-r border-solid border-primary-first border-t"></td>
            <td className="border-t border-solid border-primary-first text-right">
              {fmtInt(totalDNACount)}
            </td>
            <td className="border-t border-solid border-primary-first text-right"></td>
            <td className="border-t border-solid border-primary-first text-right">
              {fmtInt(totalRNACount)}
            </td>
            <td className="border-r border-solid border-primary-first border-t text-right"></td>
            <td className="border-r border-solid border-primary-first border-t text-right">
              {fmtInt(totalGSCount)}
            </td>
        ...
        <thead>
          <tr>
            ...
            <th className="border-t border-solid border-primary-first">
              {"N\u00A0DNA"}
            </th>
            <th className="border-t border-solid border-primary-first">
              {"%\u00A0DNA"}
            ...
            <th className="border-t border-r border-solid border-primary-first">
              {"N\u00A0GS"}
            </th>
            <th
              className="border-b border-solid border-primary-first bg-[#FBEDDA]"
          ...
          <tr>
            <th className="border-r" colSpan={5}>
              Number of steps in
            </th>
          </tr>
        </thead>
  ...
  render() {
    return (
      <div className="h-full mx-auto overflow-hidden" style={{ flex: 1 }}>
        ...
        >
          <div className="overflow-hidden">
            <div className="rdo-scroll-vertically">{this.renderList()}</div>
          </div>
                ...
                <div className="flex">
                  <DownloadButtonComponent
                    ...
                    onClick={() =>
                      Net.serveFile(
                        "text/plain",
                        ListOfConformers.raw,
                        "list_of_conformers.csv"
                      )
                    }
                  />
                  <DownloadButtonComponent
                    ...
                    onClick={() =>
                      Net.serveFile(
                        "application/json",
                        JSON.stringify(ListOfConformers.list),
                        "list_of_conformers.json"
                      )
                    }
                  />
                </div>
      ...
      case "base-pairs":
        return (
          <>
            ...
            <div>
              You will be redirected to a new page by clicking on this button,
              and none of your work will be lost
            </div>
          </>
        );
      case "table-of-conformers":
      ...
      <div className="rdo-offset">
        <div className="rdo-screen-with-side-panel overflow-hidden h-full flex flex-col">
          ...
          <div className="flex flex-col overflow-hidden mx-4 mb-4">
            ...
            <div className="overflow-scroll">{this.renderTab()}</div>
          </div>
        </div>
      </div>
    );
