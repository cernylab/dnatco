...
import {
  ...
  SelectionDisplayer,
  SelectedPieces,
  StructureSelectionFromViewer,
  StructureSelectionSwitching,
...
function masterModeViews(
  mode: MasterMode
): [id: ViewId, item: { caption: string }][] {
  return ViewsInMode[mode].map(([viewId, _]) => [
    viewId,
    { caption: AvailableViews[viewId].caption },
  ]);
}

const ExcludeModelIndex = ["modelIndex"] as (keyof StructureSelection)[];
const ExcludeModelIndexAndChain = [
  "modelIndex",
  "chain",
] as (keyof StructureSelection)[];
...
function Inner(props: {
  ...
  routeElems.push({
    path: "*",
    element: (
      <ViewWrapper view={ViewsInMode[props.mode.master][0][1]} {...props} />
    ),
  });

  const view = ViewsInMode[props.mode.master].find(
    ([viewId, _]) => props.mode.viewId === viewId
  )![1];
  ...
  return (
    <>
      <ViewsList
        views={views}
        onSwitchView={(viewId) =>
          navigate(`/app/dnatco/${props.mode.master}/${viewId}`)
        }
        selected={props.mode.viewId}
      />
      <div className="flex flex-col overflow-hidden">
        <DynamicSplitView
          containerClass="rdo-view-visualizer-container flex-1 overflow-hidden"
            ...
            const tryRedraw = (attempt: number) => {
              ...
              if (props.viewerInterop.ready())
                ...
                Logger.log(
                  ...
                  `Viewer was not ready on attempt ${attempt + 1}`
                );
    ...
    return (
      <div className="flex my-auto">
        ...
        <div className="flex">
          <div>(Low:{"\u00A0"}</div>
          <div>
            {getCifValue(props.d, Refine, "ls_d_res_low")?.toFixed(1) ?? "N/A"})
          </div>
        </div>
      </div>
    );
...
function StructureCaption(props: { d: Dnatcofication }) {
  return (
    <div className="structure-caption flex flex-col justify-center mb-2 ml-4">
      <div className="flex">
        <div className="text-18px font-700 mr-2">{props.d.identifyingName}</div>
        <div className="text-18px font-700">{props.d.identifyingTitle}</div>
      </div>
      <div className="flex">
...
export function MainScreen(props: {
  ...
  const structureSelection = React.useMemo<StructureSelection>(
    () =>
      StructureSelectionFromViewer(props.viewerInterop, props.dnatcofication),
    []
  );
  ...
  const switchChain = async (chain: string) => {
    ...
    if (chain === InvalidChain)
      props.viewerInterop.api.command(
        ViewerApi.Commands.Filter(Filters.Empty())
      );
    else {
      const model =
      ...
      const filter = Filters.Slices([
        { chain: model.chains.find((x) => x.name === chain)!.authName },
      ]);
  ...
  const switchModel = async (modelIndex: number) => {
    ...
    if (modelIndex !== InvalidModelIndex) {
      const modelNum =
        props.dnatcofication.data.structures[0].models[modelIndex].num;
      await props.viewerInterop.api.command(
        ViewerApi.Commands.SwitchModel(modelNum)
      );
    }
  ...
  const updateViewer = (
    ...
    if (view.visualizer) {
      ...
      if (view.granularity !== "dont-care")
        vi.api.command(
          ViewerApi.Commands.SwitchSelectionGranularity(view.granularity)
        );
      vi.api.command(ViewerApi.Commands.Freeze(false));
    } else vi.api.command(ViewerApi.Commands.Freeze(true));
  };
  ...
  const [dnatcoMode, setDnatcoMode] = React.useState(
   ...
   */
  const dumb = React.useMemo(
    () =>
      new SelectedNtCSetDumbWorkaroundWrapperDataClassMemoizableItem(
        selectedCustomNtCSet
      ),
    []
  );
  ...
  React.useEffect(() => {
    ...
    subs.push(
      props.dnatcofication.customNtCs.events.setsCleared.subscribe(() =>
    ...
    subs.push(
      props.dnatcofication.customNtCs.events.setRenamed.subscribe(
        ({ oldName, newName }) => {
          if (dumb.set === oldName) setSelectedCustomNtCSet(newName);
        }
      )
    );
    subs.push(
      props.dnatcofication.customNtCs.events.setChanged.subscribe(
        ({ set, step }) => {
          if (set !== dumb.set) return;
    ...
    props.viewerInterop
      ...
      .then(() => {
        subs.push(
          props.viewerInterop.events.residueRequested.subscribe((residue) => {
            const mode = locationToDnatcoMode(navPath(window.location)); // See the BEWARE above
            const view = Register.Views[mode.viewId];
            if (view.granularity !== "residue") return;

            const r = StructureSelection.authToCifResidue(
              props.dnatcofication.data.structures[0],
              residue
            );
            if (r) {
              const pieces = view.selectionMaker(
        ...
        subs.push(
          props.viewerInterop.events.structuresDeselected.subscribe(() => {
            structureSelection.steps.splice(0, structureSelection.steps.length);
            structureSelection.residues.splice(
              0,
        ...
        subs.push(
          props.viewerInterop.events.ready.subscribe(() => {
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-secondary-third">
          Something went wrong during initialization of the interactive viewer.
          You may try to reload the page and try again...
          <br />
          {initializationError}
        </div>
      </div>
