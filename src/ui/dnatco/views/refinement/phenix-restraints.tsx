...
function renderLines(lines: Phenix.Line[]) {
  if (lines.length > 10000)
    return (
      <div className="rdo-emphasize text-center">
        Configuration is too long to be displayed. You can download it as a text
        file by clicking on the &ldquo;Download&rdquo; button
      </div>
    );
  return lines.map((line, idx) => {
    const text = replaceAll(line.text, LeadingWS, "\u00A0");
    if (line.isOk) return <div key={idx}>{text}</div>;
    else
      return (
        <div className="text-secondary-third" key={idx}>
          {text}
        </div>
      );
  });
...
export class PhenixRestraints extends View<Refinement.Props, State> {
  ...
  render() {
    const restraints = Phenix.restraints(
      ...
      <div className="overflow-hidden h-full flex flex-col">
        <div>
          <div className="rdo-secondary-caption">
            Restraints for NtC-aware Phenix
          </div>
        ...
        <div className="rdo-offset overflow-scroll flex-1">
          {renderLines(lines)}
        </div>
        <div>
          Note that this restraints file requires a modified
          &ldquo;NtC-aware&rdquo; version of Phenix. Contact the authors of the{" "}
          {GlobalConfig.data().displayedProductName} website for further
          information.
        </div>
      </div>
    );
