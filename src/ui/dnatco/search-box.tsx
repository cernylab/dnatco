...
function reducer<T>(state: State<T>, action: ClearAction | SetAction<T>): State<T> {
    switch (action.kind) {
        case 'clear':
            return { ...state, results: [], searchPrompt: '' };
        case 'set':
            return { ...state, results: action.results, searchPrompt: action.searchPrompt };
    }
}
...
export namespace SearchBox {
    export interface Searching<T> {
        onRenderResult: (result: T, onUseResult: (result: T) => void) => JSX.Element,
